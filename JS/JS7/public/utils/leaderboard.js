...
window.addEventListener("load", () => {
  ...
  //Cuando se hace click en el boton de tabla
  $leaderboardBtn.on("click", () => {
    //Si la tabla esta oculta 
    if (!shown) {
      //Animacion con gsap de aparecion/desaparicion
      gsap.to($game, {
        opacity: 0,
        duration: 0.3,
        onComplete: () => {
      ...
      //Hacer fetch a los usuarios y generar la tabla dinamicamente
    ...
    `;
      ...
      //Si la tabla no esta oculta ocultarla con animacion
      gsap.to($leaderboard, {
        opacity: 0,
        duration: 0.3,
        onComplete: () => {
  ...
  //Funcion para conseguir usuarios
  ...
  //Funcion para formatear la ultima vez jugada
