...
export default function CallToAction() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // 1. Animación del core central con explosión inicial
      mainTl
        .fromTo(
          coreRef.current,
          { scale: 0, rotation: 0 },
      ...
      // 3. Formas geométricas con entrada dramática
      ...
      // 4. Líneas de conexión con efecto magnético
      ...
      // 5. Flecha direccional con bounce
      mainTl.fromTo(
        arrowRef.current,
      ...
      // 6. Call to action con fade elegante
      mainTl.fromTo(
      ...
      shapesRef.current.forEach((shape, index) => {
        ...
        // Movimiento orbital alrededor del core
        gsap.to(shape, {
          motionPath: {
            path: "M0,0 Q30,30 0,60 Q-30,30 0,0",
            autoRotate: false,
          },
          duration: 6 + index,
          repeat: -1,
      ...
      // Líneas con efecto de electricidad
      linesRef.current.forEach((line, index) => {
        gsap.to(line, {
          opacity: 0.2,
      ...
      // Flecha con movimiento magnético hacia abajo
      ...
      // Call to action con efecto de respiración
  ...
  return (
    ...
    >
      ...
      {/* Core central con múltiples capas */}
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        {[...Array(8)].map((_, i) => (
          <div
            ...
            style={{
              boxShadow:
                i % 2 === 0 ? "0 0 10px rgba(74, 222, 128, 0.6)" : "none",
            }}
          />
        ))}
      </div>

      {/* Red de conexiones energéticas */}
      {[...Array(12)].map((_, i) => (
        <div
          key={i}
      ...
      >
        <div className="flex items-center space-x-3 px-4 py-2 rounded-full bg-grass/10 backdrop-blur-sm border border-grass/30 group-hover:bg-grass/20 transition-all duration-300">
          ...
          <span className="text-white/90 text-sm font-regular tracking-wide">
            Descubre quien soy
          </span>
